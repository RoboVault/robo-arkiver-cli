...
import { z, Input, Secret, createClient } from "../deps.ts";
...
export const validateEmail = (email: string) => {
  ...
  if (!emailValidation.success) {
    const errorMsg = `Error parsing email: ${emailValidation.error.issues.map(
      (issue) => issue.message
    )}`;
...
export const validatePassword = (password: string) => {
  ...
  if (!passwordValidation.success) {
    const errorMsg = `Error parsing password: ${passwordValidation.error.issues.map(
      (issue) => issue.message
    )}`;
    throw new Error(errorMsg);
  }
